...
class topology():
    ...
    def gen_clockpath(self):
        ...
        with open("./symcts/clockpath.txt",'w') as f:
    ...
    def buffering(self):
        ...
        with open("./symcts/solution.txt",'r') as f:
            for i in range(len(self.num_branchs)):
        ...
        for a_wire in self.wires:
            if a_wire.startpoint == endpoint or a_wire.endpoint == startpoint:
                ...
                if own_end_point and own_start_point:
                    ...
                    own_end_point = False
        self.eraseNodesById(startpoint)
        self.eraseNodesById(endpoint)
    ...
    def exportNetlist(self):
        ...
        # # #
        self.eraseSameNodeId()
        with open('./evaluation/output/result','w') as f:
        ...
        with open('./evaluation/output/watchpoint.json','w') as f:
    ...
    def eraseSameNodeId(self):
        unique_ids = set()
        nodes_to_keep = []
        
        for node in self.nodes:
            if node.get_id() not in unique_ids:
                unique_ids.add(node.get_id())
                nodes_to_keep.append(node)
        self.nodes = nodes_to_keep
    def eraseNodesById(self, target_id):
        self.nodes = [node for node in self.nodes if node.get_id() != target_id]

    def exportStatistics(self):
        with open('./evaluation/output/statistics.txt','w') as f:
